// Employee Notes JavaScript
let currentUserId = null;
let allNotes = [];

// Load user data from localStorage
document.addEventListener('DOMContentLoaded', function() {
    // Employee uses 'employeeUser' key
    const userData = localStorage.getItem('employeeUser');
    console.log('User data from localStorage (employeeUser):', userData);
    ...
    if (userData) {
        try {
            ...
            if (userName) userName.textContent = user.full_name || 'Employee';
            if (userAvatar) userAvatar.textContent = (user.full_name || 'E').charAt(0).toUpperCase();
        } catch (e) {
...
function displayNotes(notes) {
    ...
    if (notes.length === 0) {
        container.innerHTML = '<div class="empty-state">No notes found. Click "New Note" to send a message to admin.</div>';
        return;
    }
    ...
    const details = `
        <div class="note-details">
            ...
            <div class="note-actions">
                ${note.sender_type === 'admin' ? '<button class="btn btn-primary" onclick="replyToNote(' + note.id + ')">Reply</button>' : ''}
...
function replyToNote(noteId) {
    ...
    const note = allNotes.find(n => n.id === noteId);
    if (note) {
        document.getElementById('noteSubject').value = `Re: ${note.subject || ''}`;
        document.getElementById('noteMessage').value = `\n\n--- Original Message ---\nFrom: ${note.sender_type === 'admin' ? 'Admin' : note.sender_name}\nDate: ${new Date(note.created_at).toLocaleString()}\n\n${note.message}`;
    }
}
...
document.getElementById('composeForm').addEventListener('submit', async function(e) {
    ...
    try {
        ...
        const adminResponse = await fetch('../../backend/api/admin.php?action=get_admin_id');
        const adminResult = await adminResponse.json();
        
        let adminId = 1; // fallback
        if (adminResult.success && adminResult.admin_id) {
        ...
        if (result.success) {
            Swal.fire({
                icon: 'success',
                title: 'Success',
                text: 'Note sent to admin successfully',
                confirmButtonColor: '#10b981'
            });
            closeComposeModal();
            loadNotes();
